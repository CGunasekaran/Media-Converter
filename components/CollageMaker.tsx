...
type LayoutType = 'grid-2' | 'grid-3' | 'grid-4' | 'instagram' | 'facebook' | 'custom';

const LAYOUTS = [
  { value: 'grid-2' as LayoutType, label: '2 Images (Grid)', width: 800, height: 400, slots: 2 },
  { value: 'grid-3' as LayoutType, label: '3 Images (Grid)', width: 900, height: 300, slots: 3 },
  { value: 'grid-4' as LayoutType, label: '4 Images (Grid)', width: 800, height: 800, slots: 4 },
  { value: 'instagram' as LayoutType, label: 'Instagram Post', width: 1080, height: 1080, slots: 4 },
  { value: 'facebook' as LayoutType, label: 'Facebook Cover', width: 820, height: 312, slots: 3 },
];
...
export default function CollageMaker() {
  ...
  const drawGrid2 = useCallback((ctx: CanvasRenderingContext2D, imgs: HTMLImageElement[]) => {
  ...
  const drawGrid3 = useCallback((ctx: CanvasRenderingContext2D, imgs: HTMLImageElement[]) => {
  ...
  const drawGrid4 = useCallback((ctx: CanvasRenderingContext2D, imgs: HTMLImageElement[]) => {
    const w = (selectedLayout.width - spacing) / 2;
  ...
  const drawFacebookCover = useCallback((ctx: CanvasRenderingContext2D, imgs: HTMLImageElement[]) => {
  ...
  }, [images, layout, backgroundColor, selectedLayout, drawGrid2, drawGrid3, drawGrid4, drawFacebookCover]);
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-violet-500/20 to-purple-500/20 border border-violet-300 dark:border-violet-700 rounded-lg p-4">
        <p className="text-sm text-white font-medium">
          <strong>🎨 Collage Maker:</strong> Combine multiple images into beautiful layouts for social media
        </p>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-4 bg-white/50 dark:bg-slate-800/50 p-6 rounded-lg backdrop-blur-sm">
          <h3 className="text-lg font-bold text-white mb-4">⚙️ Collage Settings</h3>
          ...
          {images.length > 0 && (
            <div className="space-y-2">
              ...
              {images.map((img, i) => (
                <div key={i} className="flex items-center gap-2 bg-white dark:bg-slate-700 p-2 rounded">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={img} alt={`Image ${i + 1}`} className="w-12 h-12 object-cover rounded" />
                  <span className="flex-1 text-sm text-gray-700 dark:text-gray-200">Image {i + 1}</span>
                  <button
