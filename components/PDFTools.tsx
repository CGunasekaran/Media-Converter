...
type PDFOperation = 'merge' | 'split' | 'rotate' | 'compress' | 'extract-text' | 'add-page-numbers';
...
export default function PDFTools() {
  ...
  const downloadResult = () => {
    ...
    fetch(result.url)
      .then(res => res.blob())
      .then(blob => {
        const filename = operation === 'merge' ? 'merged.pdf' : 
                        operation === 'split' ? 'split.pdf' : 
                        operation === 'rotate' ? 'rotated.pdf' :
                        operation === 'compress' ? 'compressed.pdf' :
                        operation === 'add-page-numbers' ? 'numbered.pdf' : 'output.pdf';
    ...
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-blue-500/20 to-indigo-500/20 border border-blue-300 dark:border-blue-700 rounded-lg p-4">
        <p className="text-sm text-white font-medium">
          <strong>📄 PDF Tools:</strong> Powerful utilities to manipulate and manage your PDF documents
        </p>
      </div>

      <div className="bg-white/50 dark:bg-slate-800/50 p-6 rounded-lg backdrop-blur-sm">
        <h3 className="text-lg font-bold text-white mb-4">🛠️ Select Operation</h3>
        ...
        <div className="mb-4">
          ...
          >
            📁 Select PDF {operation === 'merge' ? 'Files' : 'File'} ({files.length} selected)
          </button>
        </div>

        {files.length > 0 && (
          <div className="mb-4 p-3 bg-white dark:bg-slate-700 rounded">
            <p className="text-sm font-medium text-gray-700 dark:text-gray-200 mb-2">Selected Files:</p>
          ...
          {operation === 'add-page-numbers' && (
            <div>
              ...
              <select
                ...
                onChange={(e) => setPageNumberPosition(e.target.value as 'bottom-center' | 'bottom-right' | 'top-right')}
                className="w-full p-2 border rounded text-gray-700"
              >
          ...
          {operation === 'compress' && (
            <div>
              ...
              <input
                ...
                className="w-full"
              />
              <p className="text-xs text-white mt-1">Lower = smaller file size, higher = better quality</p>
            </div>
          )}
        </div>
      ...
      {result && (
        <div className="bg-white/50 dark:bg-slate-800/50 p-6 rounded-lg backdrop-blur-sm">
          ...
          {result.text ? (
            <div>
              <p className="text-sm font-medium text-white mb-2">Extracted Text:</p>
              <div className="bg-white dark:bg-slate-700 p-4 rounded max-h-96 overflow-y-auto">
                <pre className="text-sm text-gray-700 dark:text-gray-200 whitespace-pre-wrap">{result.text}</pre>
              </div>
              <button
                onClick={() => {
            ...
            <div>
              <p className="text-sm text-white mb-4">PDF processed successfully!</p>
              <button
